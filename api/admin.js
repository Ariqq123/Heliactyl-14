...
module.exports.load = async function (app, db) {
  ...
  app.get("/remove_account", async (req, res) => {
    ...
    if (!pteroid)
      return res.redirect(
        theme.settings.redirect.removeaccountfailed +
          "?err=REMOVEACCOUNTINVALIDID"
      );

    let targetaccount = await fetch(
      settings.pterodactyl.domain +
        "/api/application/users/" +
        pteroid +
        "?include=servers",
    ...
    if (targetaccount.ok) {
      let targetaccountinfo = JSON.parse(await targetaccount.text());
      if (targetaccountinfo.attributes.relationships.servers.data.length > 0)
        return res.redirect(
          theme.settings.redirect.removeaccountfailed +
            "?err=REMOVEACCOUNTHASSERVERS"
        );
    }
